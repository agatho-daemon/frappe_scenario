...
class ScenarioTroubleshooting {
		...
		if (!this.run_name) {
			this.body.html(
				`<div class="alert alert-warning">${__(
					"Open the lab from a completed Scenario Run."
				)}</div>`
			);
			return;
		}
	...
	render() {
		...
		$(`<div class="mb-4"><h3>${frappe.utils.escape_html(this.model.run.company)}</h3>
			<p class="text-muted">${__(
				"Diagnose one controlled ERPNext problem at a time. The lab never corrects the problem silently."
			)}</p></div>`).appendTo(this.body);
	...
	render_problem(problem) {
		...
		const card = $(`<section class="card mb-3"><div class="card-body">
			<div class="text-muted small">${frappe.utils.escape_html(
				problem.module
			)} · ${frappe.utils.escape_html(problem.difficulty)}</div>
			...
			<button class="btn btn-primary start-case" ${unavailable ? "disabled" : ""}>${__(
			"Start case"
		)}</button>
	...
	async start(problem) {
		frappe.confirm(
			__("Create a reversible training problem for {0}?", [problem.title]),
			async () => {
				await frappe.call({
			...
			.map(
				(choice) =>
					`<option value="${frappe.utils.escape_html(
						choice.key
					)}">${frappe.utils.escape_html(choice.label)}</option>`
			)
			.join("");
		const hints = active.hints.length
			? `<div class="alert alert-info mt-3">${active.hints
					.map((hint) => frappe.utils.escape_html(hint))
					.join("<br>")}</div>`
			: "";
		const card = $(`<section class="card"><div class="card-body">
			<span class="indicator-pill ${
				active.status === "Diagnosed" ? "green" : "orange"
			}">${frappe.utils.escape_html(active.status)}</span>
			<h3 class="mt-3">${frappe.utils.escape_html(active.title)}</h3>
			<p>${frappe.utils.escape_html(active.objective)}</p>
			<a class="btn btn-default mb-3" href="${encodeURI(active.target.route)}">${__(
			"Open ERPNext evidence"
		)}</a>
			<div class="form-group"><label>${__(
				"Your diagnosis"
			)}</label><select class="form-control diagnosis"><option value="">${__(
			"Select…"
		)}</option>${choices}</select></div>
	...
	async diagnose(active, card) {
		...
		frappe.show_alert({
			...
			indicator: response.message.passed ? "green" : "orange",
		});
		await this.load();
	}
	...
	restore(active) {
		frappe.confirm(
			__("Remove the training problem and restore the pre-lab checkpoint?"),
			async () => {
				const response = await frappe.call({
				...
				if (response.message.blockers.length) {
					frappe.msgprint({
						title: __("Restore blocked"),
						indicator: "orange",
						message: response.message.blockers
							.map(
								(item) =>
									`<p>${frappe.utils.escape_html(
										item.reference
									)}: ${frappe.utils.escape_html(item.message)}</p>`
							)
							.join(""),
					});
				} else {
					frappe.show_alert({
						message: __("Lab baseline restored"),
						indicator: "green",
					});
				}
				await this.load();
